...
// 清空当前请求的全局过滤规则，本次请求不再执行default_filter
        $this->request->filter([]);
...
if(Request::instance()->isPost()) {
$webinput=input("post.");
// 处理 Debug 开关：app_debug 不是 web 表字段，需写入 config.php 后剔除
if(isset($webinput["app_debug"])){
    $debugVal = $webinput["app_debug"]=="1" ? "true" : "false";
    $cfgFile = APP_PATH."config.php";
    if(is_file($cfgFile) && is_writable($cfgFile)){
        $cfg = file_get_contents($cfgFile);
        $cfg = preg_replace("/('app_debug'\s*=>\s*)(true|false)/", "$1".$debugVal, $cfg, 1);
        @file_put_contents($cfgFile, $cfg);
    }
    unset($webinput["app_debug"]);
}
...
// 读取 config.php 当前 debug 状态供页面回显
$this->web["app_debug"] = (config("app_debug") ? "1" : "0");
...
public function testmail() {
    ...
    if($mail=="" || !filter_var($mail, FILTER_VALIDATE_EMAIL)){
        return ["code"=>"-1","msg"=>"请输入正确的邮箱地址"];
    }
    $web = $this->web;
    if($web["emailname"]=="" || $web["emailhost"]==""){
        return ["code"=>"-1","msg"=>"请先填写并保存邮箱配置"];
    }
    try {
        $m = new PHPMailer(true);
        $m->IsSMTP();
        $m->CharSet   = $web["emailchar"] ? $web["emailchar"] : "utf-8";
        $m->SMTPAuth  = $web["emailauth"];
        if($web["emailsecure"]){
            $m->SMTPSecure = $web["emailsecure"];
        }
        $m->Port     = $web["emailport"];
        $m->Host     = $web["emailhost"];
        $m->Username = $web["emailname"];
        $m->Password = $web["emailpass"];
        $m->From     = $web["emailname"];
        $m->FromName = $web["name"] ? $web["name"] : "系统邮件";
        $m->AddAddress($mail);
        $m->Subject  = "【".$web["name"]."】邮箱配置测试邮件";
        $m->Body     = "<html><head><title>测试邮件</title></head><body><p>这是一封来自 ".$web["name"]." 的测试邮件。</p><p>如果你收到本邮件，说明邮箱 SMTP 配置正确。</p><p>发送时间：".date("Y-m-d H:i:s")."</p></body></html>";
        $m->WordWrap = 80;
        $m->IsHTML(true);
        $m->Send();
        return ["code"=>"1","msg"=>"测试邮件已发送，请到 ".$mail." 查收（含垃圾箱）"];
    } catch (Exception $e) {
...
public function testsms() {
        ...
        $channel  = trim(input("smschannel"));
        $id       = trim(input("smsid"));
        $key      = trim(input("smskey"));
        $sign     = trim(input("smssign"));
        $template = trim(input("smstemplate"));
        $appid    = trim(input("smsappid"));
        if($id=="" || $key=="" || $sign=="" || $template==""){
            return ["code"=>"-1","msg"=>"请先填写完整的短信接口参数（密钥/签名/模板）"];
        }
        if($channel=="tencent" && $appid==""){
            return ["code"=>"-1","msg"=>"腾讯云短信需填写短信应用 SdkAppId"];
        }
        try {
            $sms = new \sms\Sms([
                'channel'  => $channel,
            ...
            if(isset($res["code"]) && $res["code"]==1){
                return ["code"=>"1","msg"=>"测试短信已发送，请到 ".$phone." 查收（验证码：".$code."）"];
            }
            return ["code"=>"-1","msg"=>"发送失败：".(isset($res["msg"])?$res["msg"]:"未知错误")];
        } catch (\Exception $e) {
            return ["code"=>"-1","msg"=>"发送失败：".$e->getMessage()];
        }
    }
...
"state"=>$state,
"smrz"=>$smrz,
"realname"=>$smrz=="1"?$realname:"",
"idcard"=>$smrz=="1"?$idcard:"",
"smtime"=>($smrz=="1" && $data["smrz"]!="1")?time():($smrz=="1"?$data["smtime"]:"0"),
"password"=>password_hash($password,PASSWORD_DEFAULT),
]);
...
sendNotify($user["phone"],$user["mail"],"回复工单通知","站长在时间:".date("Y-m-d H:i:s")."已回复工单<br/>工单id:".$id."<br/>请及时查看!<br/><br/>","站长已回复你的工单(ID:".$id.")，请及时查看");
$array["code"]="1";
$array["msg"]="回复工单成功!";
}else{
$array["code"]="-1";
...
if($data1){
$user=Db::name('user')->where("id",$data["userid"])->find();
sendNotify($user["phone"],$user["mail"],"关闭工单通知","站长在时间:".date("Y-m-d H:i:s")."已关闭工单<br/>工单id:".$id."<br/><br/>","站长已关闭你的工单(ID:".$id.")");
$array["code"]="1";
$array["msg"]="关闭工单成功!";
}else{
$array["code"]="-1";
...
sendNotify($data3["phone"],$data3["mail"],"你的工单已被站长关闭","站长在时间:".date("Y-m-d H:i:s")."已关闭你的工单<br/>工单ID:".$data2["id"]."<br/><br/>","站长已关闭你的工单(ID:".$data2["id"].")");
}
if($data2["state"]!="4"){
$a=$a+1;
}else{
...
sendNotify($user["phone"],$user["mail"],"你的提现申请已处理","站长在时间:".date("Y-m-d H:i:s")."已处理你的提现申请<br/>提现记录ID:".$cid."<br/>请及时查看!<br/><br/>","你的提现申请(ID:".$cid.")已处理，请及时查看");
$array["code"]="1";
$array["msg"]="成功!";
}else{
$array["code"]="-1";
...
$search=input("search");
if($search){
$data=Db::name('transferrecord')->whereor("id", 'like', '%'.$search.'%')->whereor("userid", 'like', '%'.$search.'%')->whereor("record", 'like', '%'.$search.'%')->whereor("remark", 'like', '%'.$search.'%')->order('id desc')->paginate(10,false,['query'=>request()->param()]);
}else{
